...
(function () {
  ...
  // Messages
  // ------------
  const messageMarkAsReadAll = document.querySelector('.dropdown-messages-all');
  const messageMarkAsReadList = document.querySelectorAll('.dropdown-messages-read');

  // Message: Mark as all as read
  if (messageMarkAsReadAll) {
    messageMarkAsReadAll.addEventListener('click', event => {
      messageMarkAsReadList.forEach(item => {
        item.closest('.dropdown-messages-item').classList.add('marked-as-read');
      });
    });
  }
  // Message: Mark as read/unread onclick of dot
  if (messageMarkAsReadList) {
    messageMarkAsReadList.forEach(item => {
      item.addEventListener('click', event => {
        item.closest('.dropdown-messages-item').classList.toggle('marked-as-read');
      });
    });
  }

  // Message: Mark as read/unread onclick of dot
  const messageArchiveMessageList = document.querySelectorAll('.dropdown-messages-archive');
  messageArchiveMessageList.forEach(item => {
    item.addEventListener('click', event => {
      item.closest('.dropdown-messages-item').remove();
    });
  });
